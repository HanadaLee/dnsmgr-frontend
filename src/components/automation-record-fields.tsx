...
import { recordValueForDisplay } from '@/lib/dns-record-value'
...
function optionLabel(record: DnsRecord, providerType: string | undefined) {
  return `${record.name} · ${record.type} · ${recordValueForDisplay(providerType, record.type, record.value)}`
}
...
export function AutomationRecordFields({ domains, fields, values, onChange }: { domains: AutomationDomainOption[]; fields: FormFieldSpec[]; values: Record<string, unknown>; onChange: (values: Record<string, unknown>) => void }) {
  ...
  const providerType = domains.find((domain) => domain.id === domainId)?.providerType
  ...
  const recordOptions = (records.data ?? []).map((record) => ({ value: record.id, label: optionLabel(record, providerType) }))
  ...
  return <FieldGroup>
    <Field><FieldLabel>域名</FieldLabel><Select items={domains.map((domain) => ({ value: String(domain.id), label: domain.name }))} value={values.domainId ? String(values.domainId) : null} onValueChange={(value) => onChange({ ...values, domainId: value ?? '', recordId: '', recordName: '', recordType: '', primaryValue: '', currentValue: '', currentValues: undefined, lineId: '', lineLabel: '', ttl: 600 })}><SelectTrigger className="w-full"><SelectValue placeholder="请选择域名" /></SelectTrigger><SelectContent><SelectGroup>{domains.map((domain) => <SelectItem key={domain.id} value={String(domain.id)}>{domain.name}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
    <Field><FieldLabel>解析记录</FieldLabel><Select items={recordOptions} value={values.recordId ? String(values.recordId) : null} disabled={!domainId || records.isPending} onValueChange={(value) => { const record = records.data?.find((item) => item.id === value); if (!record) return; onChange({ ...values, recordId: record.id, recordName: record.name, recordType: record.type, primaryValue: recordValueForDisplay(providerType, record.type, record.values?.[0] ?? record.value), currentValue: recordValueForDisplay(providerType, record.type, record.value), currentValues: record.values?.map((item) => recordValueForDisplay(providerType, record.type, item)), lineId: record.line.id, lineLabel: record.line.label, ttl: record.ttl ?? 600 }) }}><SelectTrigger className="w-full"><SelectValue placeholder={records.isPending ? '正在读取解析记录…' : '请选择解析记录'} /></SelectTrigger><SelectContent><SelectGroup>{recordOptions.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectGroup></SelectContent></Select>{records.isPending ? <FieldDescription className="inline-flex items-center gap-1"><Spinner />正在加载全部记录</FieldDescription> : selected ? <FieldDescription>{selected.line.label} · TTL {selected.ttl ?? '—'} · {selected.status === 'enabled' ? '已启用' : '已停用'}</FieldDescription> : null}</Field>
