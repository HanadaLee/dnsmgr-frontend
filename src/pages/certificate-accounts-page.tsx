...
async function getCertificateAccountTypes(kind: CertificateAccountKind) {
  return (await apiGet<DataResponse<CertificateAccountTypeDefinition[]>>('/api/web/v1/certificate-account-types', { kind })).data
}

export function CertificateAccountCreateButton({ kind }: { kind: CertificateAccountKind }) {
  const types = useQuery({
    queryKey: ['certificate-account-types', kind],
    queryFn: () => getCertificateAccountTypes(kind),
  })

  return (
    <CertificateAccountDialog
      trigger={<Button disabled={types.isPending || types.isError}><PlusIcon data-icon="inline-start" />添加账户</Button>}
      kind={kind}
      types={types.data ?? []}
    />
  )
...
export function CertificateAccountsPage({ kind }: { kind: CertificateAccountKind }) {
  ...
  const types = useQuery({ queryKey: ['certificate-account-types', kind], queryFn: () => getCertificateAccountTypes(kind) })
