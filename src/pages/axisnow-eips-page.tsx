  ...
  const columns: DataColumn<AxisNowEip>[] = [
    {
      ...
      render: (eip) => {
        ...
        return <div className="min-w-48"><p className="flex items-center gap-2 font-medium"><CountryFlag countryCode={eip.geo.countryCode} provinceCode={eip.geo.provinceCode} tagNames={eip.tagNames} /><span>{eip.address}</span></p><p className="text-xs text-muted-foreground">{location || eip.geo.ispName || '—'}</p></div>
      },
    },
