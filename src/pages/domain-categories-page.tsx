...
export function DomainCategoriesPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      apiGet<PageResponse<DomainCategory>>("/api/web/v1/domain-categories", {
        page,
        pageSize: 20,
        sort,
        order,
  ...
  >({
    mutationFn: ({ id, body }) =>
      id
        ? apiPut(`/api/web/v1/domain-categories/${id}`, body)
        : apiPost("/api/web/v1/domain-categories", body),
    successMessage: (_, variables) =>
      variables.id ? "分类已更新" : "分类已添加",
    invalidate: [["domain-categories"], ["domains"]],
  });
  const remove = useApiMutation<number, DataResponse<OperationResult>>({
    mutationFn: (id) => apiDelete(`/api/web/v1/domain-categories/${id}`),
  ...
  const columns: DataColumn<DomainCategory>[] = [
    ...
    },
    {
      ...
      label: "域名数",
      render: (category) => (
        <span className="tabular-nums">{category.domainCount}</span>
      ),
    },
    {
      key: "sort",
      label: "排序",
      render: (category) => (
        <span className="tabular-nums">{category.sort}</span>
      ),
    },
    {
      key: "time",
      label: "添加时间",
      render: (category) => formatDateTime(category.addedAt),
    },
    {
      key: "actions",
      label: "",
      ...
      render: (category) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuGroup>
              <DropdownMenuItem
                render={<Link to={`/domains?categoryId=${category.id}`} />}
              >
                <Globe2Icon />
                查看域名
              </DropdownMenuItem>
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        eyebrow="DNS"
        title="域名分类"
        description="通过分类整理域名，并控制列表展示顺序。"
        action={
          <FormDialog
            trigger={
              <Button>
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="grid gap-2 sm:grid-cols-2 sm:justify-end lg:grid-cols-[12rem_8rem] lg:self-end">
            ...
            >
              ...
              <SelectContent>
                <SelectGroup>
                  <SelectItem value="sort">按排序值</SelectItem>
                  <SelectItem value="id">按添加顺序</SelectItem>
                  <SelectItem value="name">按分类名称</SelectItem>
