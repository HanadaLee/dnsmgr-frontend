...
async function getDomainAccountProviders() {
  return (await apiGet<DataResponse<DnsProviderDefinition[]>>('/api/web/v1/domain-account-providers')).data
}

function domainAccountLabel(account: DomainAccountSummary) {
  return account.remark?.trim() || account.name
}

export function DomainAccountCreateButton() {
  const providers = useQuery({
    queryKey: ['domain-account-providers'],
    queryFn: getDomainAccountProviders,
  })

  return (
    <AccountDialog
      trigger={<Button disabled={providers.isPending || providers.isError}><PlusIcon data-icon="inline-start" />添加账户</Button>}
...
export function DomainAccountsPage() {
  ...
  const columns: DataColumn<DomainAccountSummary>[] = [
    { key: 'name', label: '账户', render: (account) => <p className="min-w-48 font-medium">{domainAccountLabel(account)}</p> },
    ...
    { key: 'time', label: '添加时间', render: (account) => formatDateTime(account.addedAt) },
    {
      key: 'actions', label: '', className: 'w-12', render: (account) => (
        <DropdownMenu>
          <DropdownMenuTrigger render={<Button size="icon-sm" variant="ghost" aria-label={`管理 ${domainAccountLabel(account)}`} />}><MoreHorizontalIcon /></DropdownMenuTrigger>
          <DropdownMenuContent align="end"><DropdownMenuGroup>
            ...
            <ConfirmAction
              ...
              title={`删除 ${domainAccountLabel(account)}？`} description="仅无关联域名的账户可以删除，此操作无法撤销。" destructive pending={remove.isPending} onConfirm={() => remove.mutate(account.id)}
            />
    ...
    <Card>
      <CardContent className="flex flex-col gap-4">
        <form className="grid gap-2 md:grid-cols-[minmax(14rem,1fr)_12rem_8rem_auto]" onSubmit={(event) => { event.preventDefault(); setPage(1); setQueryText(search.trim()) }}>
          <div className="relative flex-1"><SearchIcon className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索账户标识或备注" /></div>
          <Select items={[{ value: 'id', label: '按添加顺序' }, { value: 'provider', label: '按服务商' }, { value: 'name', label: '按账户标识' }, { value: 'remark', label: '按备注' }, { value: 'addedAt', label: '按添加时间' }]} value={sort} onValueChange={(value) => { setSort(value ?? 'id'); setPage(1) }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="id">按添加顺序</SelectItem><SelectItem value="provider">按服务商</SelectItem><SelectItem value="name">按账户标识</SelectItem><SelectItem value="remark">按备注</SelectItem><SelectItem value="addedAt">按添加时间</SelectItem></SelectGroup></SelectContent></Select>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto sm:max-w-xl">
        <form onSubmit={(event) => {
          ...
          mutation.mutate({ providerType, config, remark: remark || null }, { onSuccess: () => setOpen(false) })
        }}>
          ...
          <DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>取消</Button><Button type="submit" disabled={mutation.isPending || !providerType}>{mutation.isPending ? <Spinner data-icon="inline-start" /> : null}保存</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  )
