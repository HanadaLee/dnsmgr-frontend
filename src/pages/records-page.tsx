...
function recordPayload(values: Record<string, unknown>, providerType?: string) {
  const type = String(values.type ?? '')
  return {
    ...values,
    value: recordValueForSave(providerType, type, String(values.value ?? '')),
...
function RecordsContent({ domainId }: { domainId: number }) {
  ...
  const records = useQuery({
    ...
    queryFn: () => apiGet<PageResponse<DnsRecord>>(`/api/web/v1/domains/${domainId}/records`, { page, pageSize: 20, q: searchBy === 'keyword' ? queryText : undefined, subdomain: searchBy === 'subdomain' ? queryText : undefined, value: searchBy === 'value' ? recordValueForSave(options.data?.providerType, typeFilter === 'CNAME' ? 'CNAME' : undefined, queryText) : undefined, type: typeFilter === 'all' ? undefined : typeFilter, line: lineFilter === 'all' ? undefined : lineFilter, groupId: groupFilter === 'all' ? undefined : groupIdFromSelect(groupFilter), status: statusFilter === 'all' ? undefined : statusFilter, sort, order }),
  ...
  const remove = useApiMutation<DnsRecord, DataResponse<OperationResult>>({ mutationFn: (record) => apiDelete(`/api/web/v1/domains/${domainId}/records/${encodeURIComponent(record.id)}`, { current: snapshot(record, options.data?.providerType) }), successMessage: '解析记录已删除', invalidate: [...invalidate] })
  const status = useApiMutation<DnsRecord, DataResponse<OperationResult>>({ mutationFn: (record) => apiPatch(`/api/web/v1/domains/${domainId}/records/${encodeURIComponent(record.id)}/status`, { enabled: record.status !== 'enabled', current: snapshot(record, options.data?.providerType) }), successMessage: '记录状态已更新', invalidate: [...invalidate] })
  ...
  const displayValue = (record: DnsRecord) => recordValueForDisplay(options.data?.providerType, record.type, record.value)
  const columns: DataColumn<DnsRecord>[] = [
    ...
    { key: 'value', label: '记录值', render: (record) => <div className="flex max-w-md items-center gap-1"><code className="min-w-0 flex-1 truncate" title={displayValue(record)}>{displayValue(record)}{record.type === 'MX' && options.data?.providerType !== 'huawei' ? ` | ${record.mxPriority ?? 1}` : ''}</code><Button size="icon-sm" variant="ghost" aria-label={`复制 ${record.name} 的记录值`} onClick={() => { void navigator.clipboard.writeText(displayValue(record)); toast.add({ title: '记录值已复制', type: 'success' }) }}><CopyIcon /></Button></div> },
    ...
    { key: 'actions', label: '', className: 'w-12', render: (record) => <RecordActions record={record} domainName={domain.data?.name} options={options.data} updatePending={update.isPending} remarkPending={remark.isPending} deletePending={remove.isPending} onUpdate={(body, close) => update.mutate({ id: record.id, body: { ...body, current: snapshot(record, options.data?.providerType) } }, { onSuccess: close })} onRemark={(nextRemark, close) => remark.mutate({ id: record.id, remark: nextRemark }, { onSuccess: close })} onCheck={() => check.mutate(record)} onDelete={() => remove.mutate(record)} /> },
  ]
    ...
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="records">
        ...
        <TabsContent value="records">
          {options.data?.capabilities.hierarchicalRecords ? <QingCloudRecords domainId={domainId} options={options.data} /> : <Card><CardContent className="flex flex-col gap-4">
...
function RecordActions({ record, domainName, options, updatePending, remarkPending, deletePending, onUpdate, onRemark, onCheck, onDelete }: { record: DnsRecord; domainName?: string; options?: RecordOptions; updatePending: boolean; remarkPending: boolean; deletePending: boolean; onUpdate: (body: Record<string, unknown>, close: () => void) => void; onRemark: (remark: string | null, close: () => void) => void; onCheck: () => void; onDelete: () => void }) {
  ...
  return <DropdownMenu><DropdownMenuTrigger render={<Button size="icon-sm" variant="ghost" aria-label={`管理 ${record.name}`} />}><MoreHorizontalIcon /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuGroup>
    <FormDialog trigger={<DropdownMenuItem closeOnClick={false}><PencilIcon />编辑</DropdownMenuItem>} title="编辑解析记录" fields={recordFields(options)} initialValues={{ name: record.name, type: record.type, value: recordValueForDisplay(options?.providerType, record.type, record.value), lineId: record.line.id, ttl: record.ttl ?? 600, mxPriority: record.mxPriority ?? 1, weight: record.weight ?? 0, mode: record.mode ?? 1, remark: record.remark ?? '' }} pending={updatePending} onSubmit={(values, close) => onUpdate(recordPayload(values, options?.providerType), close)} />
    {options?.capabilities.recordRemark === 'separate' ? <FormDialog trigger={<DropdownMenuItem closeOnClick={false}><PencilIcon />编辑备注</DropdownMenuItem>} title="编辑记录备注" fields={[{ name: 'remark', label: '备注', kind: 'textarea' }]} initialValues={{ remark: record.remark ?? '' }} pending={remarkPending} onSubmit={(values, close) => onRemark(String(values.remark ?? '') || null, close)} /> : null}
...
function BatchToolbar({ records, options, groups, pending, onBatch }: { records: DnsRecord[]; options?: RecordOptions; groups: RecordGroup[]; pending: boolean; onBatch: (body: Record<string, unknown>, close: () => void) => void }) {
  const snapshots = records.map((record) => snapshot(record, options?.providerType))
  const actions = [
